...
function WorkspaceState(props) {
	...
	workspaces.forEach((workspace, index) => {
		...
		workspace.sessionIds.forEach((sessionId) => {
			...
			if (session?.pendingInteraction !== void 0) pending = true;
		...
		if (pending) rules.push(`${row}{color:var(--ds-warning,#d99a22);animation:dsh-workspace-status-pending 2s ease-in-out infinite;transform-origin:center}`);
		else if (active) rules.push(`${row}{animation:dsh-workspace-status-pulse 1.6s ease-in-out infinite;transform-origin:center;color:var(--ds-accent,#4f9cff)}`);
