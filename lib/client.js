...
*/
function titleIndex(sessions, archivedSessionIds) {
	const archived = new Set(archivedSessionIds);
	const index = /* @__PURE__ */ new Map();
	for (const [sessionId, session] of Object.entries(sessions)) {
		if (session === void 0 || session.blank === true || session.origin === "subagent") continue;
		if (archived.has(sessionId)) continue;
		const title = session.displayTitle;
		if (title === void 0 || title === "") continue;
...
function WorkspaceState(props) {
	...
	const byTitle = (0, react.useMemo)(() => titleIndex(sessions, archivedSessionIds), [sessions, archivedSessionIds]);
