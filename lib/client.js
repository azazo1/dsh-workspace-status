...
*/
function workspaceStatus(workspace, sessions, sessionStatuses, jobsBySession) {
	const status = {
		pending: false,
		active: false,
		unread: false,
...
//#endregion
//#region src/client/row-identity.ts
...
*/
function rowSessionId(element, sessions) {
	const key = element.getAttribute("data-row-key");
	if (key === null || !key.startsWith(SESSION_ROW_KEY_PREFIX)) return void 0;
	const sessionId = key.slice(8);
	const session = sessions[sessionId];
	return session === void 0 || session.blank === true ? void 0 : sessionId;
}
...
function titleElement(row) {
	return Array.from(row.children).find((child) => child.tagName === "SPAN" && child.children.length === 0 && !child.hasAttribute("data-owner") && (child.textContent?.trim() ?? "") !== "");
}
...
function sync(snapshot) {
	...
	if (snapshot.liveJobSessions.size > 0) for (const row of document.querySelectorAll(ROW_SELECTOR)) {
		const sessionId = rowSessionId(row, snapshot.sessions);
		if (sessionId === void 0 || !snapshot.liveJobSessions.has(sessionId)) continue;
	...
	for (const marker of document.querySelectorAll(`[data-owner="${MARKER_OWNER}"]`)) if (!kept.has(marker)) marker.remove();
}
...
function WorkspaceState(props) {
	...
	const watchedRef = (0, react.useRef)(/* @__PURE__ */ new Map());
	(0, react.useEffect)(() => {
	...
	(0, react.useEffect)(() => () => {
		const watched = watchedRef.current;
		watchedRef.current = /* @__PURE__ */ new Map();
		for (const release of watched.values()) release();
	}, []);
	const statuses = workspaces.map((workspace) => workspaceStatus(workspace, sessions, sessionStatuses, jobsBySession));
