...
- 0.1.7-rc.2 新增的 `sidebar.session.row.leading` 只在该行主状态为 idle 时渲染, 与行自身的状态点互斥; 本插件的蓝点需要和三种行状态同时成立, 因此仍通过 DOM 注入. 从 React 行组件的 key 与 `node.id` / `result.id` 交叉确认 session id, 支持同名分叉, 改名和列表重排, 不按标题反查.
